...
public class FormReferencial extends JPanel {
    ...
    private void setValorCampo(CampoConfig cc, String valorABuscar) {
        JComponent comp = campos.get(cc.columnaBD);
        if (comp instanceof JTextField) {
            ((JTextField) comp).setText(valorABuscar);
        } else if (comp instanceof JComboBox) {
            JComboBox<String> combo = (JComboBox<String>) comp;
            boolean encontrado = false;
            for (int i = 0; i < combo.getItemCount(); i++) {
                String item = combo.getItemAt(i);
                if (item.startsWith(valorABuscar + " | ") || item.equals(valorABuscar)) {
    ...
    private void cargarOpcionesFK(JComboBox<String> combo, CampoConfig cc) {
        ...
        try (Connection con = Conexion.getConexion(); Statement st = con.createStatement(); ResultSet rs = st.executeQuery(sql)) {
            combo.addItem(""); // Opción vacía por defecto
            while (rs.next()) {
    ...
    private void cargarGrilla() {
        ...
        sql.append(config.columnaEstado).append(" FROM ").append(config.tabla).append(" ORDER BY ").append(config.campoClave().columnaBD);
    ...
    private void ejecutarUpdate(Connection con) throws SQLException {
        ...
        try (PreparedStatement ps = con.prepareStatement("UPDATE " + config.tabla + " SET " + set + " WHERE " + config.campoClave().columnaBD + " = ?")) {
            int idx = 1;
            for (CampoConfig cc : config.campos) if (cc.editableEnModificar) ps.setString(idx++, getValorCampo(cc));
            ps.setString(idx, getValorCampo(config.campoClave()));
            ps.executeUpdate();
        }
    }

    private void ejecutarEstado(Connection con) throws SQLException {
        try (PreparedStatement ps = con.prepareStatement("UPDATE " + config.tabla + " SET " + config.columnaEstado + " = ? WHERE " + config.campoClave().columnaBD + " = ?")) {
            ps.setString(1, txtEstado.getText().trim());
            ps.setString(2, getValorCampo(config.campoClave()));
            ps.executeUpdate();
        }
    }
    ...
    private void cargarFilaEnRegistro() {
        int i = 0;
        for (CampoConfig cc : config.campos) setValorCampo(cc, modeloTabla.getValueAt(filaSeleccionada, i++).toString());
        txtEstado.setText(modeloTabla.getValueAt(filaSeleccionada, i).toString());
    }

    private boolean hayFilaSeleccionada() {
        if (filaSeleccionada < 0) { JOptionPane.showMessageDialog(this, "Seleccione un registro.", "Aviso", JOptionPane.WARNING_MESSAGE); return false; }
        return true;
    }
